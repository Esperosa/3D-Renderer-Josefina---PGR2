...
public class PathTracerRenderer implements Renderer {
    ...
    private int denoiseStartSamples = 12;
    private int denoiseRadius = 1;
    private double denoiseStrength = 0.24;
    ...
    public synchronized void setParameter(String key, Object value) {
        ...
        if (("maxdepth".equals(k) || "maxbounces".equals(k) || "depth".equals(k)) && value instanceof Number) {
            maxBounces = Math.max(1, Math.min(16, ((Number) value).intValue()));
            resetAccumulation();
            return;
        }
        ...
        if ("denoisestartsamples".equals(k) && value instanceof Number) {
            denoiseStartSamples = Math.max(1, Math.min(100000, ((Number) value).intValue()));
            return;
        }
    ...
    private void tracePath(TraceContext ctx, SplitMix64 rng) {
        double ox = ctx.rayOx;
        double oy = ctx.rayOy;
        double oz = ctx.rayOz;
        double dx = ctx.rayDx;
        ...
        for (int bounce = 0; bounce < maxBounces; bounce++) {
            ...
            radianceB += throughputB * ctx.surface.sheenB * sheenWeight * 0.22;
            ...
            if (directLighting) {
                ...
                for (DirLightCache light : dirLights) {
                    ...
                    lightR += baseR * light.r * nDotL * diffuseProbability;
                    lightG += baseG * light.g * nDotL * diffuseProbability;
                    lightB += baseB * light.b * nDotL * diffuseProbability;
                    lightR += ctx.surface.specR * light.r * spec * Math.max(0.08, specProbability);
                    lightG += ctx.surface.specG * light.g * spec * Math.max(0.08, specProbability);
                    lightB += ctx.surface.specB * light.b * spec * Math.max(0.08, specProbability);
                }
                ...
                for (PointLightCache light : pointLights) {
                    ...
                    lightR += baseR * light.r * att * nDotL * diffuseProbability;
                    lightG += baseG * light.g * att * nDotL * diffuseProbability;
                    lightB += baseB * light.b * att * nDotL * diffuseProbability;
                    lightR += ctx.surface.specR * light.r * att * spec * Math.max(0.08, specProbability);
                    lightG += ctx.surface.specG * light.g * att * spec * Math.max(0.08, specProbability);
                    lightB += ctx.surface.specB * light.b * att * spec * Math.max(0.08, specProbability);
                }

                radianceR += throughputR * lightR;
                radianceG += throughputG * lightG;
                radianceB += throughputB * lightB;
            }
            ...
            double specularThreshold = transmissionThreshold + specProbability;
            if (branchPick < transmissionThreshold && transmissionProbability > 1e-6) {
                ...
                if (refracted.lengthSquared() < 1e-10) {
                    ...
                    specProbability = clamp01(specProbability + transmissionThreshold);
                    diffuseProbability = clamp01(1.0 - specProbability);
                    branchPick = transmissionThreshold + 1e-3;
                } else {
                    nextDx = refracted.x;
                    nextDy = refracted.y;
                    nextDz = refracted.z;
                    double tint = clamp01(ctx.surface.density * Math.max(0.05, ctx.surface.thickness) + transmissionThreshold * 0.25);
                    double invBranchPdf = 1.0 / Math.max(1e-6, transmissionThreshold);
                    throughputR *= mix(1.0, ctx.surface.mediumR, tint) * invBranchPdf * transmissionThreshold;
                    throughputG *= mix(1.0, ctx.surface.mediumG, tint) * invBranchPdf * transmissionThreshold;
                    throughputB *= mix(1.0, ctx.surface.mediumB, tint) * invBranchPdf * transmissionThreshold;
            ...
            if (branchPick < specularThreshold && specProbability > 1e-6) {
                ...
                nextDy = ctx.sampleDy;
                nextDz = ctx.sampleDz;
                double safeDiffuseProb = Math.max(1e-6, diffuseProbability);
                double invBranchPdf = 1.0 / safeDiffuseProb;
